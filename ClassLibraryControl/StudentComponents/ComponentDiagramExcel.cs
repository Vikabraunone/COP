...
namespace ClassLibraryControl.Components
{
    public partial class ComponentDiagramExcel : Component
    {
        ...
        public void CreateDiagram(string fileName, List<StudentViewModel> students)
        {
            ...
            var groups = students.GroupBy(x => x.FormEducation);
            int row = 0;
            foreach (var group in groups)
            {
                row++;
                // записываем форму обучения в 1 столбце
